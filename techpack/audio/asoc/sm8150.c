...
static struct snd_soc_dai_link quat_mi2s_rx_tas256x_dai_links[] = {
	{
		.name = LPASS_BE_QUAT_MI2S_RX,
		.stream_name = "Quaternary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.3",
...
static struct snd_soc_dai_link msm_tavil_dai_links[
			 ...
			 ARRAY_SIZE(ext_disp_be_dai_link) +
			 ARRAY_SIZE(msm_mi2s_be_dai_links) +
			 ARRAY_SIZE(quat_mi2s_rx_tas256x_dai_links) +
			 ARRAY_SIZE(msm_auxpcm_be_dai_links)];
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	if (!strcmp(match->data, "pahu_codec")) {
		...
		if (!ret && val) {
			...
			total_links += ARRAY_SIZE(msm_mi2s_be_dai_links);
			memcpy(msm_tavil_dai_links + total_links,
				quat_mi2s_rx_tas256x_dai_links,
				sizeof(quat_mi2s_rx_tas256x_dai_links));
			total_links += ARRAY_SIZE(quat_mi2s_rx_tas256x_dai_links);
		}
